...
declare module "moo" {
  ...
  export function compile<T>(rules: Rules<T>): Lexer<T>;

  export function states<T>(
    states: { [x: string]: Rules<T> },
    start?: string
  ): Lexer<T>;
  ...
  export interface Lexer<T = string> {
    /**
     * Returns a string with a pretty error message.
     */
    formatError(token: Token<T>, message?: string): string;
    /**
